...
public class GamblerMain
{
	...
	public static void Get30DaysReport() {
		...
		do{
			...
			{
				int overallLoss=totalLossInMonth-totalWinInMonth;
				System.out.println("overall loss in a month is "+overallLoss);
			}

			else {
				int overallWin=totalWinInMonth-totalLossInMonth;
				System.out.println("overall profit in a month is "+overallWin);
			}
			GetLuckyAndUnluckytDay(arr);

		}while(totalLossInMonth<totalWinInMonth);
	}
